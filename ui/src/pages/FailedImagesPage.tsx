...
export default function FailedImagesPage() {
  ...
  const [promptScans, setPromptScans] = useState<Scan[]>([]);
  ...
  useEffect(() => {
    ...
    loadAllPolicyExceptionScans(apiClient, {
      inUseRequestParams,
      repository,
      policyFilter,
    })
  ...
  }, [apiClient, inUseRequestParams, repository, policyFilter]);
    ...
    <div>
      ...
      {totalScans > 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4 items-start">
          <div className="lg:col-span-2 flex items-center gap-3 p-4 rounded-xl border border-warning/30 bg-warning-bg/20">
          ...
          <PolicyAgentPromptCard scans={promptScans} hint={POLICY_AGENT_PROMPT_HINT_FILTERED} />
        </div>
      )}
