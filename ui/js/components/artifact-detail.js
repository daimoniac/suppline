 ...
 * Displays scan details for an artifact within a repository context.
 * Used when navigating from repositories -> repository detail -> artifact.
 * Maintains proper breadcrumb navigation back to the repository.
 */

import { ScanDetailBase } from './scan-detail-base.js';

export class ArtifactDetail extends ScanDetailBase {
    ...
    /**
     * Render the complete artifact detail view with repository context
     */
    render() {
        if (!this.scan) {
    ...
    async loadData() {
        ...
        // For ArtifactDetail, we need the digest parameter, so we override loadAndRender instead
    }
    ...
    async loadAndRender(digest) {
        try {
            this.showLoading('Loading artifact details...');
