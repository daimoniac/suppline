...
// @Param in_use_mode query string false "Image usage: all (default), in_use, not_in_use, in_use_newer" Enums(all,in_use,not_in_use,in_use_newer)
...
func (s *APIServer) handleListScans(w http.ResponseWriter, r *http.Request) {
	...
	imageUsage := parseImageUsageQuery(r)
...
// @Param limit query int false "Maximum number of results" default(100)
// @Success 200 {array} types.VulnerabilityGroup
// @Failure 401 {object} map[string]string "Unauthorized"
// @Failure 500 {object} map[string]string "Internal server error"
// @Security BearerAuth
...
func (s *APIServer) handleQueryVulnerabilities(w http.ResponseWriter, r *http.Request) {
	...
	encoder := json.NewEncoder(w)
	if err := encoder.Encode(pagedGroups); err != nil {
		s.logger.Error("error encoding JSON response",
			"error", err,
			"path", r.URL.Path)
	}
...
// @Param in_use_mode query string false "Tag list: all / not_in_use (show all tags), in_use, in_use_newer" Enums(all,not_in_use,in_use,in_use_newer)
// @Param limit query int false "Maximum number of results" default(100)
// @Param offset query int false "Pagination offset" default(0)
...
func (s *APIServer) groupVulnerabilitiesByCVE(vulnerabilities []*types.VulnerabilityRecord, sortBy, sortDir string, maxDigests int, includeDigests bool) []*types.VulnerabilityGroup {
	...
	// To keep track of affected repositories/digests within each CVE
	// map[CVEID] -> map[Repository] -> map[Digest] -> details
	...
	for _, v := range vulnerabilities {
		...
		cveTracking := tracking[v.CVEID]
		if _, exists := cveTracking.repos[v.Repository]; !exists {
			cveTracking.repos[v.Repository] = make(map[string]*digestDetail)
		}
		if _, exists := cveTracking.repos[v.Repository][v.Digest]; !exists {
			cveTracking.repos[v.Repository][v.Digest] = &digestDetail{
	...
	// Build the final structure from the map
