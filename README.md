...
- **Sigstore Attestations** - SBOM, vulnerability, VEX, and SCAI attestations published as OCI Sigstore bundle referrers with cosign
...
- Cosign key pair for attestations (Cosign v2.4+ recommended for OCI bundle format)
- ORAS CLI (optional, for referrer discovery checks)
...
```bash
# Verify attestations generated by suppline (new OCI Sigstore bundle format)
cosign verify-attestation --type https://in-toto.io/attestation/scai/attribute-report/v0.3 --key keys/cosign.pub --new-bundle-format --insecure-ignore-tlog myprivateregistry/alpine:3.22 | jq -r .payload | base64 -d | jq -r

# Validate bundle referrers are present for the attested digest
DIGEST="$(crane digest myprivateregistry/alpine:3.22)"
oras discover --format json "myprivateregistry/alpine@${DIGEST}" | jq .
...
```
You can also check the attestations of type `cyclonedx` and `vuln` that suppline generates for each digest. The `oras discover` output should include Sigstore bundle referrers.
